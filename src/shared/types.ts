...
// Action Queue Types
export type ActionItemStatus = 'pending' | 'processing'

export type ActionResult = 'approved' | 'rejected' | 'dismissed' | 'retried'

export interface ActionQueueItem {
  id: string
  notificationId: string // Original notification ID (for linking)
  type: 'awaiting' | 'error'
  sessionId: string
  title: string
  body: string
  timestamp: number // Arrival time (for FIFO sorting)
  status: ActionItemStatus
  groupKey?: string // Group key for batch processing (same messages)
  agent?: HookAgentType // which agent type (claude, gemini, cursor)
  toolName?: string // tool requesting permission

  // HITL metadata (unified permission request info)
  hitlRequest?: HITLRequest
}

export interface ActionHistoryItem {
  id: string
  originalItem: ActionQueueItem
  result: ActionResult
  processedAt: number
  processedBy: 'user' | 'bulk' // Individual vs bulk processing
}
