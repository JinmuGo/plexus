...
class HistoryCaptureManager {
  ...
  private async parseSessionJsonl(session: ClaudeSession): Promise<void> {
    ...
    try {
      ...
      // Add tool executions from JSONL
      for (const tool of toolExecutions) {
        // Check if we already have this tool execution
        const existingTools = historyStore.getToolExecutions(session.id)
        const exists = existingTools.some(t => t.toolUseId === tool.toolUseId)

        if (!exists && tool.toolUseId) {
