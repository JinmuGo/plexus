...
function extractQuestionText(
  ...
  const options = Array.isArray(toolInput.options)
    ? (toolInput.options as string[]).filter(o => typeof o === 'string')
    : undefined

  const header =
    typeof toolInput.header === 'string' ? toolInput.header : undefined
 ...
 */
function _getDisplayTitle(cwd: string): string {
  ...
  private cleanupStaleSessions(): void {
    ...
    for (const [sessionId, session] of this.sessions) {
      ...
      if (session.agent === 'cursor') {
        ...
        // Activity-based stale detection for Cursor (longer threshold)
        if (inactivityTime > CURSOR_INACTIVITY_THRESHOLD_MS) {
          console.log(
            `[SessionStore] Marking inactive Cursor session ${sessionId.slice(0, 8)} as ended (${Math.round(inactivityTime / 1000)}s inactive)`
          )
          this.updatePhase(sessionId, 'ended')
        }
        continue
        ...
        if (session.pid) {
          if (this.isProcessAlive(session.pid)) {
            ...
            if (session.phase === 'compacting') {
              console.log(
                `[SessionStore] Compacting timed out for ${sessionId.slice(0, 8)}, resetting to waitingForInput`
              )
