...
/**
 * Get the TTY device path for the parent process (Cursor IDE)
 */
function getTty(): string | undefined {
  try {
    const result = execSync(`ps -p ${process.ppid} -o tty=`, {
      encoding: 'utf-8',
      timeout: 2000,
    }).trim()
...
function mapEventToStatus(eventName: string, input: CursorHookInput): string {
  switch (eventName) {
    ...
    case 'stop':
      // All stop statuses (completed, aborted, error) treated as session end
 ...
 */
async function readStdin(): Promise<CursorHookInput> {
  return new Promise((resolve, reject) => {
    ...
    process.stdin.on('end', () => {
      try {
        const input = JSON.parse(data) as CursorHookInput
        resolve(input)
      } catch (error) {
        reject(error)
      }
