...
const QUESTION_TOOLS = new Set(['AskUserQuestion', 'AskFollowupQuestion'])
...
function getTty(): string | undefined {
  ...
  try {
    if (process.stdin.isTTY) {
      // Node.js doesn't have a direct way to get tty path
      // But if we have a TTY, return a placeholder
      return undefined
    }
  } catch {
    // Ignore errors
