...
// Session event type for renderer (legacy IPC-based)
interface SessionEvent {
  type: 'add' | 'update' | 'remove' | 'output'
...
const API = {
  ...
  sessions: {
    ...
    onEvent: (callback: (event: SessionEvent) => void): (() => void) => {
      const handler = (_: Electron.IpcRendererEvent, event: SessionEvent) => {
        callback(event)
      }
  ...
  permissions: {
    ...
    onRequest: (
      callback: (sessionId: string, context: PermissionContext) => void
    ): (() => void) => {
      const handler = (
        _: Electron.IpcRendererEvent,
        data: { sessionId: string; context: PermissionContext }
      ) => {
        callback(data.sessionId, data.context)
      }
    ...
    onResolved: (
      callback: (sessionId: string, toolUseId: string) => void
    ): (() => void) => {
      const handler = (
        _: Electron.IpcRendererEvent,
        data: { sessionId: string; toolUseId: string }
      ) => {
        callback(data.sessionId, data.toolUseId)
      }
      ipcRenderer.on('permission:resolved', handler)
      return () => ipcRenderer.removeListener('permission:resolved', handler)
    },
  },
  ...
  activity: {
    ...
    // Event: new activity entry
    onNew: (
      callback: (sessionId: string, entry: SessionActivityEntry) => void
    ): (() => void) => {
      const handler = (
        _: Electron.IpcRendererEvent,
        data: { sessionId: string; entry: SessionActivityEntry }
      ) => {
        callback(data.sessionId, data.entry)
      }
